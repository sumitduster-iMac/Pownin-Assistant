...
class MistralProvider: AIModelProvider {
    ...
    init(apiKey: String? = nil, model: String = "mistral-small-latest") {
        self.apiKey = apiKey ?? ProcessInfo.processInfo.environment["MISTRAL_API_KEY"]
        self.model = model
    }
    ...
    func generateResponse(prompt: String, context: MessageContext) async throws -> String {
        ...
        guard httpResponse.statusCode == 200 else {
            throw AIModelError.networkError(NSError(domain: "Mistral", code: httpResponse.statusCode))
        }
