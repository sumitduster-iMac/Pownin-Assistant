...
//  Pownin-Assistant
//
//  GitHub Copilot model provider (via GitHub Models)
//

import Foundation

class CopilotProvider: AIModelProvider {
    let name = "GitHub Copilot"
    private let apiKey: String?
    private let model: String
    private let baseURL = "https://api.githubcopilot.com/chat/completions"
    ...
    init(apiKey: String? = nil, model: String = "gpt-4o") {
        self.apiKey = apiKey ?? ProcessInfo.processInfo.environment["GITHUB_TOKEN"]
        self.model = model
    }
    ...
    func generateResponse(prompt: String, context: MessageContext) async throws -> String {
        ...
        guard httpResponse.statusCode == 200 else {
            throw AIModelError.networkError(NSError(domain: "GitHub", code: httpResponse.statusCode))
        }
