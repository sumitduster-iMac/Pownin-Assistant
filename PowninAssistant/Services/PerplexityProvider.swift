...
//  Pownin-Assistant
//
//  Perplexity AI model provider
//

import Foundation

class PerplexityProvider: AIModelProvider {
    let name = "Perplexity AI"
    ...
    init(apiKey: String? = nil, model: String = "llama-3.1-sonar-small-128k-online") {
        self.apiKey = apiKey ?? ProcessInfo.processInfo.environment["PERPLEXITY_API_KEY"]
        self.model = model
    }
    ...
    func generateResponse(prompt: String, context: MessageContext) async throws -> String {
        ...
        guard httpResponse.statusCode == 200 else {
            throw AIModelError.networkError(NSError(domain: "Perplexity", code: httpResponse.statusCode))
        }
